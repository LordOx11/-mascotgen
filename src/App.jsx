...
- 🚫 THE STORY NEVER TALKS ABOUT ITSELF. Never write "the chapter", "this story", "the reader", "the narrator", or any sentence describing what is or is not being revealed — lines like "the chapter does not say who" or "that is a question for another time" are forbidden. When something is meant to stay unknown, simply DO NOT WRITE IT: leave the thing undescribed and move on. Withholding is invisible; the moment you announce it, the spell breaks and it reads like an author making excuses.
- 🚫 SAY IT ONCE. Never introduce the same object, figure or fact twice as though it were new. If a watcher at the edge of the lot has already been described, later mentions must treat it as ALREADY KNOWN ("the watcher hasn't moved"), never re-introduce it ("one figure goes very still and watches"). Before writing any sentence, check whether this chapter has already said it — and if it has, either cut the sentence or advance it.
- 📏 A PANEL IS A PARAGRAPH, NOT A PAGE — THIS IS A HARD CAP, AND THE LAST PANEL IS NOT EXEMPT. 3 to 5 sentences, roughly 60–90 words, never more. Count them. A closing panel that runs to nine sentences breaks this rule exactly as much as any other panel; if the ending needs more room, end it earlier — the strongest last line is almost always two sentences before where you wanted to stop. Panels render as one solid block on a phone, so a long panel is a wall of text no matter how good the writing is. When a moment wants to run long, SPLIT IT INTO TWO PANELS instead of packing one — more short panels always beats fewer long ones. Cut asides, cut second qualifications, cut the third clause: one observation per sentence, then move.
- 🎭 NO STOCK TROPES ON REPEAT: NEVER write a mysterious unnamed onlooker — no "one person at the edge of the lot goes very still", no silent watcher, no figure nobody else notices, no observer fixed on the character's power. This is an absolute ban, not a preference: it has appeared in four consecutive chapters and it is the single most repeated cliché in this engine. If a scene needs an observer, that person is NAMED, has a reason to be there, and speaks. (The ONE adopted exception is Ondrea Tallow — and she only exists in a chapter when the creator names her in the request box. Never introduce her on your own.) And NPC names must never echo an existing cast member's name (no "Corse" in a world that has Corso Vell) — pick names that sound nothing like anyone established.
- 🕐 TIME ONLY MOVES FORWARD. Track the time of day across panel headers and never contradict it: if a panel said NIGHT, no later panel may have sunset light, amber skies or "the last of the light". Read your own earlier headers before writing each new one.`;
  ...
  const expandCharacter = async (mode, overrideInput) => {
    ...
    try {
      ...
      if (mode === "fight") {
        ...
        } SHAPE — one DISTINCT moment per panel, never two panels on the same beat: ① BEFORE — where, why, and a real exchange of dialogue before anything is thrown. ② FIRST EXCHANGE — who is better, shown fast. ③ THE TURN — one genuine reversal. ④ THE FINISH — decisive, exactly per the spine. ⑤ AFTER (mandatory final panel) — the human beat: what it cost, who says what now, no fighting in it. HARD RULES: every panel contains spoken dialogue — trash talk, strain, or one line that means something. Pure action description gets at most TWO sentences per panel; spend the rest on the people. SPECTACLE COMES FROM THE CHARACTER SHEET, NOT FROM THE GENRE: use every power, aura, weapon and signature move this character actually HAS — established powers should absolutely be on the page and look impressive. But never invent abilities nobody established, and never lend a character's powers to an opponent who has none: if an ordinary person is in this fight, they fight like an ordinary person while the powered character does not. A named signature move may be called out only when it belongs to that character's established kit. ONE NEW MOMENT PER PANEL: a fight may absolutely stay in one location — brawls do — but every panel must contain something that did not exist in the previous one (a new injury, a new tactic, a change in who is winning, someone new speaking). What is banned is the same beat written twice with a padded heading.
🔢 NO RUNNING TALLIES. Do not track money, debts, counts or scores across panels, and never narrate arithmetic — a character may mention a single figure once for flavour, but the chapter must never depend on numbers adding up, because they will not. Cut every subtotal. Deaths ARE allowed — if anyone dies, apply the purgatory / cosmic-waterfall time-warp rules exactly.`;
        // 6-8 short panels, not 5 long ones: forcing every beat into a single
        // panel made each panel a ~200-word wall. Beats may span two panels;
        // the per-panel length cap in STORY_VOICE does the real work.
        panelSpec = "6 to 8 SHORT panels covering the five numbered beats in order (a beat may span two panels; AFTER is always the final panel)";
      } else if (mode === "custom") {

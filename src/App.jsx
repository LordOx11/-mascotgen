...
- 🚫 THE STORY NEVER TALKS ABOUT ITSELF. Never write "the chapter", "this story", "the reader", or any line describing what is or isn't being revealed ("the chapter does not say who"). To keep something unknown, simply don't write it.
- 🚫 SAY IT ONCE. Never introduce the same object, figure or fact twice as if it were new. Later mentions treat it as already known, or get cut.
- 📏 PANEL LENGTH IS A HARD CAP, LAST PANEL INCLUDED: 3–5 sentences, 60–90 words. Panels render as one block on a phone, so long panels are walls. If a moment needs more room, SPLIT IT INTO TWO PANELS. Cut asides and second qualifications.
- 🎭 NEVER write a mysterious unnamed onlooker — no silent watcher, no figure nobody notices, no observer fixed on a character's power. Absolute ban; it is this engine's most repeated cliché. Any observer is NAMED, has a reason to be there, and speaks. (Sole exception: Ondrea Tallow, and only when the creator names her in the request box.) NPC names must never echo an established cast name.
- 🕐 TIME ONLY MOVES FORWARD. Never contradict an earlier panel's time of day — after NIGHT there is no sunset light or "last of the light".`;

...
- 📏 A PANEL IS A PARAGRAPH, NOT A PAGE — THIS IS A HARD CAP. 3 to 5 sentences, roughly 60–90 words, never more. Panels render as one solid block on a phone, so a long panel is a wall of text no matter how good the writing is. When a moment wants to run long, SPLIT IT INTO TWO PANELS instead of packing one — more short panels always beats fewer long ones. Cut asides, cut second qualifications, cut the third clause: one observation per sentence, then move.
- 🎭 NO STOCK TROPES ON REPEAT: do not add an unexplained watching figure at the edge of the scene unless the creator's request asks for one — it has become a tic. And NPC names must never echo an existing cast member's name (no "Corse" in a world that has Corso Vell) — pick names that sound nothing like anyone established.`;

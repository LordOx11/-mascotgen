...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "verify-mint") {
      ...
      // cold start.
      //
      // ⚠️ THIS PATH DEPENDS ON THE rpc-websockets OVERRIDE IN package.json.
      // umi-bundle-defaults pulls in @solana/web3.js, which pulls in
      // rpc-websockets v9 — ESM-only, and Vercel's serverless runtime
      // require()s it, producing "require() of ES Module .../rpc-websockets/"
      // and a 500 on every call. package.json pins rpc-websockets to 7.11.0
      // (the last CommonJS release) to fix it. If that override is ever
      // removed, this action starts 500ing again and mints silently fall back
      // to the two-signature flow.
